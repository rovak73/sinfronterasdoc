...
(function($) {
  ...
  vc.AddElementBlockViewBackendEditor = vc.AddElementBlockView.extend({
    ...
    createElement:function (e) {
      var model, column, row, page_section, section_column;
      _.isObject(e) && e.preventDefault();
      this.do_render = true;
      var tag = $(e.currentTarget).data('tag');
      if (this.model === false) {
           if (tag == 'mk_page_section') {
           
            page_section = vc.shortcodes.create({shortcode:'mk_page_section'});
            section_column = vc.shortcodes.create({shortcode:'vc_column', params:{width:'1/1'}, parent_id:page_section.id, root_id:page_section.id });
